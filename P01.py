...
def create_additional_features(df):
    ...
    # Jockey and trainer performance features
    for entity_type in ['jockey', 'trainer']:
        if entity_type in df.columns:
            # Calculate win rates but avoid using current race result
            entity_stats = []
            
            for idx in df.index:
                entity_name = df.loc[idx, entity_type]
                
                # Get all other races for this entity (excluding current race)
                other_races = df[
                    (df[entity_type] == entity_name) & 
                    (df.index != idx)
                ]
                
                if len(other_races) >= 5:  # Only use if enough history
                    win_rate = other_races['label'].mean()
                    race_count = len(other_races)
                else:
                    win_rate = df['label'].mean()  # Use overall average
                    race_count = 0
                
                entity_stats.append({
                    'index': idx,
                    f'{entity_type}_win_rate': win_rate,
                    f'{entity_type}_races': race_count
                })
            
            # Convert to DataFrame and merge
            stats_df = pd.DataFrame(entity_stats).set_index('index')
            df[f'{entity_type}_win_rate'] = stats_df[f'{entity_type}_win_rate']
            print(f"  Added feature: {entity_type}_win_rate")
...
def prepare_features_and_labels(df):
    ...
    # Label should already be created in create_additional_features
    ...
    # CRITICAL: Remove 'lost_by_length' as it's data leakage (only known after race)
    drop_cols = ['race_position', 'label', 'race_date', 'first_place_found', 
                 'batch_number', 'url_index_in_batch', 'source_file', 'lost_by_length', 'race_class']
    ...
    available_cat_features = ['track_name', 'race_time', 'going', 'horse_name', 'jockey', 'trainer']
    available_num_features = ['distance', 'claims', 'age', 'weight', 'rating',
                             ...
                             'track_win_rate', 'track_avg_distance', 'weight_percentile',
                             'is_young_horse', 'is_prime_age', 'is_veteran', 'race_class_encoded']
...
def train_tabnet_model(X_train, y_train, X_val, y_val, cat_features, feature_df):
    """Train TabNet model with heavy focus on finding winners."""
    print("\nCalculating HEAVY class weights to prioritize finding winners...")
    ...
    # Train TabNet
    print("\nTraining TabNet with HEAVY focus on finding winners...")
...
def save_model_and_components(model, encoders, scaler, feature_columns, cat_features, num_features, 
    ...
    Args:
        ...
        auc_score: Model AUC score for metadata
        best_f1_threshold: Best threshold for F1 score
        best_profit_threshold: Best threshold for profit
...
def load_saved_model(model_path):
    ...
    Returns:
        dict: Dictionary containing loaded model and components
    """
...
def plot_threshold_analysis_full_range(y_val, y_proba):
    ...
    """
    # CHANGE 1: Full range from 0.0 to 1.0 with smaller steps
    ...
    ax1.set_xlim(0.0, 1.0)  # CHANGE 2: Full x-axis range
    ...
    ax2.set_xlim(0.0, 1.0)  # CHANGE 3: Full x-axis range
    ...
    ax3.set_xlim(0.0, 1.0)  # CHANGE 4: Full x-axis range
    ...
    ax4.set_xlim(0.0, 1.0)  # CHANGE 5: Full x-axis range
    ...
    # Also show the extremes
    print(f"\nExtreme thresholds:")
    print(f"Threshold 0.00: Recall={recalls[0]:.3f}, Precision={precisions[0]:.3f}, F1={f1_scores[0]:.3f}")
    print(f"Threshold 1.00: Recall={recalls[-1]:.3f}, Precision={precisions[-1]:.3f}, F1={f1_scores[-1]:.3f}")
    ...
    # CHANGE 6: Return more detailed results
    return {
...
def main():
    ...
    """
    print("🏇 Starting Horse Racing Prediction Pipeline...")
    print("=" * 60)
    ...
    try:
        ...
        # Step 3: Create additional features (includes creating label column)
        print("\n🔧 STEP 3: Creating additional features...")
        df = create_additional_features(df)
        print("✅ Additional features created")
        
        # Step 4: Add advanced features (requires label column to exist)
        ...
        # Step 6: Encode and scale features
        print("\n🔄 STEP 6: Encoding and scaling features...")
        ...
        # Step 7: Split data
        print("\n✂️ STEP 7: Splitting data into train/validation sets...")
        ...
        print("\n🤖 STEP 8: Training TabNet model...")
        model = train_tabnet_model(X_train, y_train, X_val, y_val, cat_features, feature_df)
        print("✅ Model training completed")
        
        # Step 9: Make predictions and evaluate
        print("\n📊 STEP 9: Making predictions and evaluating...")
        ...
        print("\n📈 STEP 10: Running threshold analysis...")
        ...
        # Step 11: Save the model and components
        print("\n💾 STEP 11: Saving model and components...")
...
# Example usage for loading a saved model
def example_load_and_predict():
    ...
    model_path = "models/horse_racing_model_20241215_143022"  # Replace with actual path
    
    try:
        loaded_components = load_saved_model(model_path)
        model = loaded_components['model']
        encoders = loaded_components['encoders']
        scaler = loaded_components['scaler']
        metadata = loaded_components['metadata']
        
        print(f"Loaded model trained on: {metadata['training_timestamp']}")
        print(f"Model AUC: {metadata['auc_score']:.4f}")
        print(f"Best thresholds: F1={metadata['best_f1_threshold']:.3f}, Profit={metadata['best_profit_threshold']:.3f}")
        
        # Now you can use the model for predictions on new data
        # new_predictions = model.predict_proba(new_data)[:, 1]
        
    except FileNotFoundError:
        print("Model not found. Train a model first using main().")
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print(f"loaded_components = load_saved_model('{results['model_save_path']}')")
    else:
